...
import { Campaign_groups, Campaign_groupsFilter } from "../models/campaign_groups.model";

@Injectable({
  providedIn: "root",
})
export class Campaign_groupsService {
  baseUrl = API_CONSTANTS.SECURE_BASE_URL + "/campaign_groups";

  constructor(private http: HttpClient) { }

  getAll(filterConditions: Campaign_groupsFilter = {}) {
    let urlQueryParams = "";
  ...
  create(createData: Campaign_groups) {
    return this.http.post(this.baseUrl, createData);
  }

  update(id: number, updateData: Campaign_groups) {
